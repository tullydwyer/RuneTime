...
SettingShakeThreshold::SettingShakeThreshold(Controllers::Settings& settingsController,
  ...
  lv_obj_set_style_local_line_color(animArc, LV_ARC_PART_INDIC, LV_STATE_DEFAULT, Colors::deepOrange);
...
void SettingShakeThreshold::Refresh() {
  ...
  if (calibrating == 1) {
    if (xTaskGetTickCount() - vCalTime > pdMS_TO_TICKS(2000)) {
      ...
      calibrating = 2;
      lv_obj_set_style_local_bg_color(calButton, LV_BTN_PART_MAIN, LV_STATE_CHECKED, Colors::deepOrange);
      lv_obj_set_style_local_bg_color(calButton, LV_BTN_PART_MAIN, LV_STATE_CHECKED, Colors::deepOrange);
