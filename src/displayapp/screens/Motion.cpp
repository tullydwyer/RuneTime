...
Motion::Motion(Controllers::MotionController& motionController) : motionController {motionController} {
  ...
  /*Add 3 data series*/
  ser1 = lv_chart_add_series(chart, Colors::deepOrange);
  ser2 = lv_chart_add_series(chart, Colors::green);
  ser3 = lv_chart_add_series(chart, Colors::gold);
  ...
  lv_label_set_text_fmt(label, "X #FF0000 %d# Y #00B000 %d# Z #ffd464 %d#", 0, 0, 0);
...
void Motion::Refresh() {
  ...
  lv_label_set_text_fmt(label,
                        "X #FF0000 %d# Y #00B000 %d# Z #ffd464 %d# mg",
                        motionController.X(),
                        motionController.Y(),
                        motionController.Z());
  lv_obj_align(label, nullptr, LV_ALIGN_IN_TOP_MID, 0, 10);
