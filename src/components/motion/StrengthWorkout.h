    ...
    bool Update(int16_t x, int16_t y, int16_t z, uint32_t milliseconds) {
      ...
      switch (state) {
        case State::Calibrating:
          ...
          break;
        case State::Ready:
          if (distance >= 350u * 350u) {
            state = State::Outbound;
            phaseStart = milliseconds;
          }
          break;
        case State::Outbound:
          if (distance < 250u * 250u) {
            state = State::Ready;
          } else if (milliseconds - phaseStart >= 100) {
            state = State::Returning;
          }
          break;
        case State::Returning:
          if (milliseconds - phaseStart > 8000) {
            baseline = sample;
            state = State::Calibrating;
            phaseStart = milliseconds;
          } else if (distance <= 200u * 200u && milliseconds - phaseStart >= 400) {
            ++reps;
            baseline = sample;
          ...
          break;
        case State::Cooldown:
          if (milliseconds - phaseStart >= 500) {
            state = State::Ready;
          }
          break;
      }
